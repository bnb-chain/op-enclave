...
import {L1StandardBridge} from "@opbnb-bedrock/src/L1/L1StandardBridge.sol";
import {L1CrossDomainMessenger} from "@opbnb-bedrock/src/L1/L1CrossDomainMessenger.sol";
import {CrossDomainMessenger} from "@opbnb-bedrock/src/universal/CrossDomainMessenger.sol";
import {L1ERC721Bridge} from "@opbnb-bedrock/src/L1/L1ERC721Bridge.sol";
import {OptimismMintableERC20Factory} from "@opbnb-bedrock/src/universal/OptimismMintableERC20Factory.sol";
...
contract DeployChain is Ownable {
    ...
    function initializeProxies(
        ...
        L1CrossDomainMessenger(addresses.l1CrossDomainMessenger).initialize(
            SuperchainConfig(superchainConfig),
            OptimismPortal(payable(addresses.optimismPortal)),
            SystemConfig(addresses.systemConfig)
        );

        L1StandardBridge(payable(addresses.l1StandardBridge)).initialize(
            CrossDomainMessenger(addresses.l1CrossDomainMessenger),
            SuperchainConfig(superchainConfig),
            SystemConfig(addresses.systemConfig)
        );

        L1ERC721Bridge(addresses.l1ERC721Bridge).initialize(
            CrossDomainMessenger(addresses.l1CrossDomainMessenger), SuperchainConfig(superchainConfig)
        );
